...
from .networks.blocks.raft import coords_grid, SmallUpdateBlock, BidirCorrBlock
from .networks.blocks.feat_enc import SmallEncoder
...
class Amt_s(nn.Module):
    def __init__(self, 
        ...
        if corr_lvls is None:
            raise ValueError("corr_lvls must not be None")
...
def load(ckpt_path=".",  data_parallel=False, load=True, DEVICE_IDS=[2]):
    ...
    # if train use single GPU ,use specific GPU .
    else:  
